...
ENT.InteriorLightAmperage = 4.67
ENT.CabineLightAmperage = 1.4
ENT.HeadLightsAmperage = 7
ENT.ProfileLightsAmperage = 3.5
ENT.ScheduleLightAmperage = 3.5
ENT.TurnSignalLightsAmperage = 7
ENT.RearLightAmperage = 3.5

function ENT:PreInitialize(ply,tr)
	self:SetPos(tr.HitPos+Vector(0,0,65))
end
...
function ENT:InitializeLowVoltageCircuit()
	...
	self.ElLowCircuit:BuildFromData({
		...
		{node1name = "interiorlightnode",node2name = "interiorlightnode2",
			...
			{name = "interiorlight",Resistance = 3,OnUpdate = function(amp,volt,power) self:SetInteriorLight(amp/self.InteriorLightAmperage) end},
		},
		...
		{node1name = "doorlight3node",node2name = "doorlight3node2",
			{name = "doorlight3key",State = function() return self:IsPriborButtonActive("doorlights") and self:DoorIsOpened("door4") end},
		...
		{node1name = "schedulelightnode",node2name = "schedulelightnode2",
			...
			{name = "schedulelight",Resistance = 4,OnUpdate = function(amp,volt,power) self:SetScheduleLight(amp/self.ScheduleLightAmperage) end},
		},
...
ENT.TrolleyPoleCatcherPos = Vector(-241,0,-17)
ENT.TrolleyPoleCatcherAng = Angle(0,180,0)
ENT.TrolleyPoleCatcherDist = 17
...
ENT.KneelingTime = 5
ENT.KneelingConstantMultiplier = 0.6
...
ENT.WheelsData = {
	{
		...
		Turn = true,
		Wheels = {
			{Type = "aksm321",	Pos = Vector(138.5,45,-45),		Right = false,	Height = 1,		Constant = 50000,	Damping = 1300,	RDamping = 1300,	Times = 8},
			{Type = "aksm321",	Pos = Vector(138.5,-45,-45),	Right = true,	Height = 0.25,	Constant = 50000,	Damping = 1300,	RDamping = 1300,	Times = 8},
		},
	},
	{
		Drive = true,
		Turn = false,
		Wheels = {
			{Type = "aksm321_rear",	Pos = Vector(-99.5,38,-45),		Right = false,	Height = 3,		Constant = 50000,	Damping = 1300,	RDamping = 1300,	Times = 8},
			{Type = "aksm321_rear",	Pos = Vector(-99.5,-38,-45),	Right = true,	Height = 2.25,	Constant = 50000,	Damping = 1300,	RDamping = 1300,	Times = 8},
		},
	},
}
	...
	if self.KneelingProgress!=kneelingdest then
		...
		for k,v in ipairs(self.Wheels) do
			...
			for k2,v2 in ipairs(v.Wheels) do
				...
				if dt.Right then
					Trolleybus_System.MultiplyWheelConstant(self,v2,1-(1-self.KneelingConstantMultiplier)*self.KneelingProgress)
				end
			end
		end
	end
