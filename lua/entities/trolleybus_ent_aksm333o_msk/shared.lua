...
ENT.DoorsData = {
	["door1"] = {
		model = "models/trolleybus/aksm333o/door1_msk.mdl",
	...
	["doorbtn2"] = {
		...
		func = function(self,on)
			if on then
				...
				if !self:GetSpawnSetting("cabine_handrail") then
					self:OpenDoor("door1")
				end
			end
		end,
	...
	["door1manual"] = {
		...
		func = function(self,on)
			...
			if self:DoorIsOpened("door1") then
				...
				if !self:GetSpawnSetting("cabine_handrail") then 
					self:CloseDoorWithHand("door2")
				end
			else
				self:OpenDoorWithHand("door1")
				
				if !self:GetSpawnSetting("cabine_handrail") then 
					self:OpenDoorWithHand("door2")
				end
			end
		end,
	},
...
Trolleybus_System.BuildMovingMirror(ENT,"left_mirror",Vector(319.94,49.89,12.95),Angle(0,-90,0),10,10,"models/trolleybus/aksm333o/mirror_left_msk.mdl",Vector(335.35,51.43,19.71),Angle(0,0,0),"Bone001","Bone002",Vector(-1.9,1.5,-0.3),Angle(-1,179,-1),6.7,15,false,true,-45,45,-45,45,-10,10,-10,10,nil,10,3,0,-5)
Trolleybus_System.BuildMovingMirror(ENT,"middle_mirror",Vector(334.47,6.76,14.12),Angle(0,-180,0),10,10,"models/trolleybus/aksm333o/mirror_middle_msk.mdl",Vector(330.28,-5.24,30.87),Angle(0,0,0),"Bone001","Bone002",Vector(-1.72,0,0.3),Angle(0,-180,0),12,7,true,false,-35,35,-35,35,-20,10,-20,10,nil,-23,-0,-15)
Trolleybus_System.BuildMovingMirror(ENT,"right_mirror",Vector(342.52,-29.16,9.22),Angle(0,-180,0),10,10,"models/trolleybus/aksm333o/mirror_right_msk.mdl",Vector(334.62,-52.05,22.71),Angle(0,0,0),"Bone001","Bone002",Vector(-2.1,-1.5,-0.7),Angle(2.2,177,-1),6.7,15,false,true,-45,45,-45,45,-10,10,-10,10,nil,-18,-2,0,-2)
...
ENT.SpawnSettings = {
	...
	{
		alias = "cabine_handrail",
		type = "CheckBox",
		name = "trolleybus.aksm333o.settings.cabine_handrail",
		default = true,
		preview_on = "trolleybus/spawnsettings_previews/aksm333o/cabine_handrail1.png",
		preview_off = "trolleybus/spawnsettings_previews/aksm333o/cabine_handrail2.png",
		setup = function(self,value)
			if self.IsTrailer then return end
			
			if value then
				self:AddDynamicButton("doorbtn1",{
				...
				self:AddDynamicButton("doorbtn7",{
			...
			self:AddDynamicButton("cabinedoor",{
					name = "trolleybus.aksm333o.btns.cabinedoor",
					model = {
						model = "models/trolleybus/aksm333o/partition_door.mdl",
						offset_pos = Vector(1.1,0,-39.2),
						offset_ang = Angle(0,180,0),
						poseparameter = "state",
						poseparamstart = 0,
						poseparamend = value and 0.27 or 1,
						speedmult = value and 0.37/0.27 or 0.37,
					},
					panel = {
						name = "cabinedoor",
						pos = {0,0},
		...
		unload = function(self,value)
			...
			if value then
				self:RemoveDynamicButton("doorbtn1")
				self:RemoveDynamicButton("doorbtn7")
			end
			
			self:RemoveDynamicButton("cabinedoor")
		end,
	},
	{
		alias = "engine",
